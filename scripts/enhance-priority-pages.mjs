...
const insertionPoints = {
  'cosmetic-packaging-supplier-china/index.html': '<section class="note" id="factory-evidence" style="margin-top:24px">',
  'cosmetic-packaging-moq/index.html': '<section class="note" style="margin-top:24px"><h2>Direct Answer: MOQ Is Configuration-Specific</h2>',
  'products/glass-cosmetic-bottles/index.html': '<section class="card" aria-labelledby="glass-cosmetic-route-title">',
  'products/serum-dropper-bottles/index.html': '<section class="card" aria-labelledby="dropper-route-title">',
  'products/airless-pump-bottles/index.html': '<section class="card" aria-labelledby="airless-route-title">'
};
...
const enhancements = {
  ...
  'products/perfume-bottles/index.html': `<section class="card" style="margin-top:24px" aria-labelledby="perfume-wholesale-title"><h2 id="perfume-wholesale-title">Empty Perfume Bottles Wholesale: What to Confirm</h2><p>Compare the complete empty perfume bottle set—not only the bottle unit price. Confirm the bottle, neck finish, pump, gasket, collar, cap, decoration, carton quantity, destination and reorder plan together. The correct wholesale route depends on whether you need a stock bottle, decorated stock or a custom mold.</p><ul><li>Capacity + bottle shape</li><li>Crimp or screw neck</li><li>Cap + decoration</li><li>Quantity + destination</li></ul></section>`,
  'products/glass-cosmetic-bottles/index.html': `<section class="card" style="margin-top:24px" aria-labelledby="glass-wholesale-title"><h2 id="glass-wholesale-title">Glass Cosmetic Bottles Wholesale: Confirm the Complete Set</h2><p>For a comparable glass cosmetic bottle quote, confirm the bottle, neck finish, pump, dropper, sprayer, gasket, decoration, filling method, carton quantity and destination as one configuration. A bottle-only unit price can hide the closure, sampling and packed-shipping assumptions that determine launch cost and reorder reliability.</p></section>`,
  'products/serum-dropper-bottles/index.html': `<section class="card" style="margin-top:24px" aria-labelledby="serum-wholesale-title"><h2 id="serum-wholesale-title">Serum Dropper Bottles Wholesale: Check the Dropper System</h2><p>For wholesale serum dropper bottles, quote the bottle and dropper assembly together: glass color, neck finish, gasket, bulb, collar, pipette length, target dose, decoration, carton quantity and destination. Approve draw, release, leakage, formula contact and packed shipping with production-intent samples before comparing unit prices.</p></section>`,
  'products/airless-pump-bottles/index.html': `<section class="card" style="margin-top:24px" aria-labelledby="airless-wholesale-title"><h2 id="airless-wholesale-title">Airless Pump Bottles Wholesale: Verify the Full System</h2><p>A wholesale airless bottle quote should identify piston or pouch structure, formula-contact parts, pump and actuator, fill method, priming, target dose, evacuation, decoration, refill route, carton quantity and destination. Compare the complete system with production-intent samples; the word “airless” alone does not establish compatibility, output or usable evacuation.</p></section>`,
  'cosmetic-packaging-supplier-china/index.html': `<section class="card" style="margin-top:24px" aria-labelledby="supplier-evidence-title"><h2 id="supplier-evidence-title">Cosmetic Packaging Supplier China: Request Comparable Evidence</h2><p>When comparing a cosmetic packaging supplier in China, request a matched evidence set for the selected item: legal entity and production scope, component bill of materials, itemized quote, stock or tooling status, approved samples, QC checkpoints, export packing and change-control terms. This makes factory claims, lead time, MOQ and reorder risk comparable before bulk ordering.</p></section>`,
  'cosmetic-packaging-moq/index.html': `<section class="card" style="margin-top:24px" aria-labelledby="moq-schedule-title"><h2 id="moq-schedule-title">Cosmetic Packaging MOQ: Request a Configured Schedule</h2><p>For a cosmetic packaging MOQ quote, request the minimum by SKU and process—not only one order total. Identify which bottle, closure, color batch, decoration step, tooling route, carton or assembly controls the minimum, then separate sample approval, production, inspection, packing and transport milestones. This schedule makes stock, decorated and custom routes comparable.</p></section>`
};
...
for (const [relativePath, content] of Object.entries(selectedEnhancements)) {
  ...
  if (existingPattern.test(source)) {
    ...
    const pageInsertionPoint = insertionPoints[relativePath] ?? insertionPoint;
    if (!source.includes(pageInsertionPoint)) {
      throw new Error(`Insertion point not found in ${relativePath}`);
    }
    source = source.replace(pageInsertionPoint, `${block}${pageInsertionPoint}`);
  }
  ...
  if (source !== originalSource) {
    ...
    const reviewedSource = source.replace(/Page reviewed \d{4}-\d{2}-\d{2}/, `Page reviewed ${requestedDate}`);
